...
export default function ArtistPage() {
  ...
  const artist = {
    ...
    name: 'The Weeknd',
    followers: '24.5M',
    image: 'https://placehold.co/300x300/45b7d1/white?text=TW',
    popularTracks: [
      { id: '3', title: 'Blinding Lights', duration: '3:19' },
      { id: '9', title: 'Save Your Tears', duration: '3:35' },
      { id: '10', title: 'Starboy', duration: '3:50' },
    ],
    albums: [
      { id: 'a3', name: 'After Hours', year: '2020', image: 'https://placehold.co/80x80/45b7d1/white?text=AH' },
      { id: 'a4', name: 'Dawn FM', year: '2022', image: 'https://placehold.co/80x80/6b46c1/white?text=DFM' },
    ],
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-white text-gray-800 flex flex-col dark:from-gray-900 dark:to-black dark:text-gray-100">
      ...
      <div className="bg-white shadow-sm p-4 flex items-center justify-between dark:bg-gray-800">
        ...
        <h1 className="font-semibold text-base">Artiste</h1>
        <div className="w-8 h-8"></div>
      </div>

      {/* Cover & Info */}
      <div className="p-4">
        <img src={artist.image} alt={artist.name} className="w-32 h-32 rounded-full mx-auto shadow-lg" />
        <div className="text-center mt-4">
          <h1 className="text-2xl font-bold">{artist.name}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">{artist.followers} followers</p>
        </div>
      </div>
      ...
      <div className="px-4 mb-6">
        <h2 className="font-bold text-lg mb-3">Albums</h2>
        <div className="grid grid-cols-2 gap-3">
          {artist.albums.map(album => (
            <Link key={album.id} href={`/album/${album.id}`} className="block">
              <div className="bg-white rounded-xl p-2 shadow-sm text-center hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700">
                <img src={album.image} alt={album.name} className="w-full aspect-square rounded-lg" />
                <p className="text-sm font-medium mt-1">{album.name}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-20">
        <h2 className="font-bold text-lg mb-3">Populaires</h2>
