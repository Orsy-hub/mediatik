...
export const FavoriteArtist = ({ favoriteArtist, index, onUnliked }: FavoriteArtistProps) => {
        ...
        >
          <div className="relative w-32 h-32 mx-auto">
            <div className="w-full h-full rounded-full overflow-hidden ring-4 ring-[#FF4BC3]/30">
              <img src={favoriteArtist.image} alt={favoriteArtist.name} className="w-full h-full object-cover" />
            </div>
          ...
          <h3 className="font-semibold text-sm mt-4">{favoriteArtist.name}</h3>
          <p className="text-xs text-white/50 mt-1">{favoriteArtist.followers} abonnés</p>
        </motion.div>
      );
}